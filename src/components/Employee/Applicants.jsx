...
import { fetchApplicantsByJob, fetchAllApplicants } from "../../store/jobsSlice";
...
const Applicants = () => {
  ...
  const allApplicants = useSelector((state) => state.jobs.applicants?.all || []);
  ...
  useEffect(() => {
    if (jobId && jobId !== "all") {
      dispatch(fetchApplicantsByJob({ jobId }))
        .unwrap()
        .catch((err) => toast.error("Failed to load applicants for this job"));
    } else {
      dispatch(fetchAllApplicants())
        .unwrap()
        .catch((err) => toast.error("Failed to load all applicants"));
    }
  }, [dispatch, jobId]);

  const applicantsToShow =
    jobId && jobId !== "all" ? jobApplicants : allApplicants;

  const filteredApplicants = applicantsToShow.filter((applicant) => {
