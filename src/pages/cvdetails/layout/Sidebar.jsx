...
const Sidebar = ({ role = "job_seeker" }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 w-65 min-h-screen bg-[#89b4d4] text-[#D1E9F6] p-6 transform transition-all duration-500 ease-in-out
  ...
  {sidebarItems.map(({ icon: Icon, label, path }) => (
    ...
    >
      ...
      {location.pathname === path && (
        ...
        {/* Footer (Optional) */}
        <div className="absolute bottom-6 text-xs text-white">
          <p>© 2025 {role === "employer" ? "Employer" : "Candidate"} Dashboard</p>
        </div>
      </aside>
